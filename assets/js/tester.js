...
(function(w, d) {
    ...
    var sampleData = {
        ...
        "tree": [{
            ...
            {
                "path": "docs/assets/app/script.md",
                "mode": "100644",
                "type": "blob",
            ...
            {
                "path": "docs/assets/app/styles.md",
                "mode": "100644",
                "type": "blob",
        ...
        ui: {
            ...
            sidebar: {
                builder: function(data) {
                    data = octapushDoc.helper.sideMenuDataBuilder(data);

                    function buildStructure(dMenu) {
                        var sMenu = '';

                        _o_.utility.each(dMenu, function(key, val) {
                            if (_o_.compare.isString(val)) {
                                sMenu += _o_.string.format('<li><a href="{1}"><p>{2}</p></a></li>', val, key);
                    ...
                    $('.sidebar ul.nav').html(buildStructure(data));

                    $('.sidebar .sidebar-wrapper').perfectScrollbar('destroy');
                    $('.sidebar .sidebar-wrapper').perfectScrollbar();

                    //$('.sidebar > .sidebar-wrapper > ul.nav').perfectScrollbar('');
                    //octapushDoc.ui.sidebar.copyright(settings.data.sideMenus.copyright);
                },
                copyright: function(data) {
                    $('.sidebar > .sidebar-wrapper > ul.nav').append(
                        _o_.string.template(
        ...
        helper: {
            sideMenuDataBuilder: function(data) {
                ...
                _o_.utility.each(data, function(key, val) {
                    // if ((val.type === 'blob' && _o_.string.right(val.path, 3) === '.md') || val.type === 'tree')
                    assignProperty(result, val.path, val.type === 'tree' ? {} : val.url);
                });

                result = octapushDoc.helper.dataFilter(result);
                result = octapushDoc.helper.dataCleanser(result);
                result = octapushDoc.helper.sortObject(result);
            ...
            dataFilter: function(data) {
                ...
                function checkInner(ret, d) {
                    _o_.utility.each(d, function(key, val) {
                        if (_o_.compare.isString(val) && _o_.string.right(key, 3) === '.md')
                            ret[key] = val;

                        else if (_o_.compare.isObject(val) && val !== {})
                ...
                checkInner(result, data);

                return result;
            },
            dataCleanser: function(data) {
                return data;
            },
