            ...
            document: {
                loadDocument: function(data) {
                    _o_.ajax.get({
                        ...
                        success: function(xhr) {
                            ...
                            oDoc.ui.sidebar.highlightActive($(_o_.string.format('ul.nav a[href*="{1}"]', data.title)))
                ...
                initPage: function() {
                    ...
                    if (_o_.compare.isArray(hashes) && hashes.length > 2)
                        oDoc.helper.document.loadDocument({
                            url: hashes[0],
                            title: hashes[2],
                            extension: hashes[1],
                            callback: function() {
                        ...
                        if (s.application.additionalData.initialPage.useFirstMenuItem == false) {
                            oDoc.helper.document.loadDocument({
                                url: s.application.additionalData.initialPage.customInitialPage.url,
                                title: s.application.additionalData.initialPage.customInitialPage.renderAs.title,
                                extension: s.application.additionalData.initialPage.customInitialPage.renderAs
                            });
                        }
                    }
                }
